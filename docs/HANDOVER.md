...
*Update 2026-09-15: the lexical half is now load-bearing —
`decoder/orchestrator.py` retrieves through it for every query. That makes
the missing dense half a live gap rather than a future one: §10 requires
hybrid retrieval precisely because "insurance wording is full of exact
terms and numbers that pure dense retrieval misses" and the converse also
holds, so a contradicting span phrased in different words than the query is
currently never retrieved, never entailed, and therefore can never produce
a CONFLICTING state. This shows up as a recall ceiling, not as a wrong
...
*Status as of 2026-09-15: done, except the coverage number itself.
`decoder/resolve/rules.py` was already a complete, tested translation of
§8's locked pseudocode. This pass added the response half:
`decoder/respond/answer_assembly.py` (answer-level state via
`aggregate_answer_state`, §8's labels per claim, both supporting AND
contradicting evidence rendered with document + page, named missing inputs)
and `decoder/respond/question_generation.py` (questions derived from each
claim's actual gap — a WELL_SUPPORTED claim generates none, which is how an
empty question list stays meaningful). User-facing strings live in
`decoder/respond/labels.py`, separate from the logic that selects them, per
§9.4's requirement that Hindi output not need a rewrite. "No answer path
bypasses verification" is asserted two ways in
`tests/test_orchestrator_smoke.py`: structurally (assemble_answer only
accepts ResolvedClaims, which only resolve() can construct) and
behaviourally (the drafter's prose must not appear in the returned
Answer.text). A formal branch-coverage measurement on `resolve/` has not
been run — the tests exercise every branch, but no coverage tool is wired
into CI yet.*
...
*Status as of 2026-09-15: the pipeline is wired and runs end to end —
`decoder/orchestrator.py` is no longer a stub. `PolicyDecoder.answer()`
runs intake → lexical retrieval → draft → decompose → per-span entailment
→ resolve → assemble, and `load_document()` classifies before segmenting so
a wrong-type document (§9.1) is refused specifically before any analysis.
...
downgraded to NEEDS_CONFIRMATION even when the figure was plainly in the
cited clause, because §7.2's "exact string match of the value in the span,
code not model" check existed nowhere — added as
`decoder/verify/numeric_check.numeric_value_appears_verbatim()` and wired
into the orchestrator. (2) The drafter wrote "This is stated in Passage 1",
which decompose faithfully turned into the user-facing claim "the
co-payment is stated in Passage 1" — meaningless to a reader, and in one
run it reached WELL_SUPPORTED because the digit in "Passage 5" coincidence-
matched the "5" in "5%". A concrete confident-and-wrong instance (§12's
headline harm case) generated by the system itself. Instructing the model
not to reference passages did not hold, and removing the numbering made it
invent numbers; what worked was telling it positively to write only about
what the policy provides, never about the document. This is worth
remembering as a pattern: the numeric-verbatim check is not a claim-level
guarantee, only a "this figure appears in the supporting span" check, so it
cannot by itself catch a claim whose predicate is nonsense.*
...
| SPIKE-2 | Ombudsman corpus feasibility | M0, all evaluation | Ombudsman archive RESOLVED — NOT CLEARED (2026-09-15) — see `/docs/corpus/ombudsman-feasibility.md`; cioins.co.in's Disclaimer/T&C explicitly prohibit copying/use of site content, no research-use exception. Replacement candidate found same day: NCDRC/State Consumer Commission judgments, PROMISING (18/20-case sample classified) — see `/docs/corpus/ncdrc-feasibility.md`. Real statutory basis (Copyright Act §52(1)(q)(iv) exempts tribunal judgments), checked directly against confonet.nic.in/ncdrc.nic.in and indiankanoon.org, neither of which restricts reuse. 18/18 sampled cases usable, covering every hero-scenario category plus two new schema-relevant sub-limit structures (AYUSH, overall per-illness cap). Room-category eligibility and pure flat-₹/day caps deliberately searched for and confirmed still absent from BOTH archives. Not yet fully resolved — 2 more cases to reach 20. |
